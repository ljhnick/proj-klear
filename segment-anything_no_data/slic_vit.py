...
from box_search import BruteForceBoxSearch, FractionAreaObjective
import clip
...
class SLICViT(nn.Module):
    ...
    def get_masks(self, im):
        ...
        for n in self.n_segments:
            ...
            oct_seg, areas = seg(im.astype(np.float32)/255., n, window_size= self.window_size)
    ...
    def get_mask_features(self,im):
        with torch.no_grad():
            ...
            im = self.model.preprocess(im).unsqueeze(0).to(device)
