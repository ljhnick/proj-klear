...
def generate_clip2d_SAM_SortBboxBackground_Interpolation(flag, image, output_path):
    ...
    for i in range(masks_number):
        ...
        #Interpolation
        if torch.count_nonzero(clip2d[torch.tensor(masks[i]["segmentation"])]) == 0:
            clip2d[torch.tensor(masks[i]["segmentation"])] = bboxBackground_image_clip1d_normalized.type(torch.float)
        else: #同一个mask下，可以有一半0，一半非0
            clip2d[torch.tensor(masks[i]["segmentation"])] = normalize((w1 * clip2d[torch.tensor(masks[i]["segmentation"])] + (1-w1) * bboxBackground_image_clip1d_normalized), p = 2.0, dim = -1).type(torch.float)
    np.save(output_path + flag + "_clip2d", clip2d.detach().cpu().numpy())
...
def train(input_path, method, output_path):
    image_dic = os.listdir(input_path)
    for image_path in image_dic:
        if image_path[-4:] == ".png":
            print(image_path)
            image_num = image_path[:-4]
            image = cv2.cvtColor(cv2.imread(input_path + image_num + ".png"), cv2.COLOR_BGR2RGB)
            if method == "SAM_Replacement":
                generate_clip2d_SAM_Replacement(image_num, image, output_path)
            elif method == "Sliding_window":
                generate_clip2d_Sliding_window(image_num, image, output_path)
            elif method == "SAM_Interpolation":
                generate_clip2d_SAM_Interpolation(image_num, image, output_path)
            elif method == "SAM_SortMask_Interpolation":
                generate_clip2d_SAM_SortMask_Interpolation(image_num, image, output_path)
            elif method == "SAM_SortBboxBackground_Interpolation":
                generate_clip2d_SAM_SortBboxBackground_Interpolation(image_num, image, output_path)

def test(output_path, flag, text, thr):
    ...
    saliency2d = torch.tensordot(clip2d_normalized.float(), text_clip1d_normalized.cpu().float(), dims=([2],[0])).detach().numpy()
    ...
    plt.imsave(output_path + flag + "_" + text + "_" + "saliency2d.png", saliency2d_remapped)
    ...
    plt.imsave(output_path + flag + "_" + text + "_" + "saliency2d" + "_" + str(thr) + ".png", saliency2d_remapped_thr)

if __name__ == "__main__":
    print("new")
    input_path = "/users/aren10/data/datasets/seg_all_2DCLIP_gt/mic/train/"
    method = "SAM_SortBboxBackground_Interpolation"
    output_path = "/users/aren10/data/datasets/seg_all_2DCLIP_gt/mic/train/"
    test_image_num = "r_52"
    text = "microphone"
    thr = 0.8
    train(input_path, method, output_path)
    #test("output_data/", test_image_num, text, thr)